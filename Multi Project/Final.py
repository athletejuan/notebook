...
#         print(self.yy.currentText(), self.mm.currentText(), self.dd.currentText())
    ...
    def danger(self, yyy, mmm, ddd, hhh):
        ...
        lists =[kkk[0], Accuracy]
        kkk2=KResult2_2.tolist()
        lists2=[KResult2_2[0], Accuracy2]
        kkk3=KResult3_2.tolist()
        lists3=[KResult3_2[0], Accuracy3]
...
class ThirdTab(QWidget):
    ...
    def clickMethod(self):
        ...
        df8 = pd.DataFrame([Total_money],index=["예상 편도 비용"],columns=["안내 내용"])
    ...
    def whichbtn(self):
        ...
        df2 = df2.rename(index=str, columns={"Unnamed: 1": "상품 리스트", "가격": "가격"})
        print(df2)
...
class FourthTab(QWidget):
    ...
    def clickMethod(self):
        ...
        table = pd.read_csv("여행비용.csv", encoding='cp949',engine='python')
        val=round(float(table.values[7,1])*1.3)
        pd.options.display.float_format='{:.2f}'.format
        ...
        pie2 = ([product_price, val*2,remaning])
    ...
    def clickMethod2(self):
        ...
        val2=round(float(table2.values[7,1])*1.3)
        pd.options.display.float_format='{:.2f}'.format
        remaning2 = float(money)-val2*2
        pie = ([money, val2*2,remaning2])
